...
public class BookResourceDao extends AbstractDbDao {
    ...
    private AbstractDbDao.ProcessResultSetCallback<List<BookcopyDto>> processGetBookCopiesStmRS = new ProcessResultSetCallback<List<BookcopyDto>>() {

        public List<BookcopyDto> process(ResultSet rs) throws SQLException, NamingException {
    ...
    private AbstractDbDao.ProcessResultSetCallback<List<BookDto>> processSearchStmResult
            = new ProcessResultSetCallback<List<BookDto>>() {
                ...
                public List<BookDto> process(ResultSet rs) throws SQLException, NamingException {
                    int firstRow = -1;
                    List<BookDto> resultList = null;

                    while (rs.next()) {
                        if (firstRow == -1) {
                            firstRow = rs.getInt("rowNum");
                            resultList = new ArrayList<BookDto>();
                        }
                        int id = rs.getInt("id");
